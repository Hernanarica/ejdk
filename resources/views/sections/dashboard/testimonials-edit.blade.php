<?php
	/** @var \App\Models\Testimony $testimony */
?>
	...
	<div class="relative min-h-[calc(100vh-88px)] flex flex-col justify-center py-12 sm:px-6 lg:px-8">
		...
		<div class="sm:mx-auto sm:w-full sm:max-w-md">
			<img
				...
				alt="Workflow">
			<h2 class="mt-6 text-center text-3xl tracking-tight font-bold text-gray-900">Editar testimonio</h2>
		</div>
		...
		<div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
			<div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
				<form
					class="space-y-6"
					action="{{ route('testimonials.update', [ 'id' => $testimony->id ]) }}"
					method="POST"
				>
					@csrf
					@method('PATCH')
					...
					<div>
						...
						<div class="mt-1">
							<textarea
								...
								name="description"
								class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
							>{{ old('description', $testimony->description) }}</textarea>
							@error('description')
								<x-error-notification-input>
									{{ $message }}
								</x-error-notification-input>
